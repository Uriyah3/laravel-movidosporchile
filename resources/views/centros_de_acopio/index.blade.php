				...
				<td class="text-center">
					...
					@if(Auth::check())
						@if(Auth::user()->rol->nombre == "Usuario")
							@if($centro->voluntario->pluck('usuario_id')->contains(Auth::id()) )
								<button class="btn octicon btn-danger btn-xs" data-title="Desinscribirse" data-toggle="modal" data-target="#desinscribirse" data-url="{{ url("centros_de_acopio/{$centro->id}/voluntarios/{$centro->voluntario->where('usuario_id', Auth::id())->first()->id}") }}"><img class="octicon" src="{{ URL::asset('icons/person.svg') }}"></button>
							@else
								<button class="btn octicon btn-success btn-xs" data-title="Inscribirse" data-toggle="modal" data-target="#inscribirse" data-url="{{ url("centros_de_acopio/{$centro->id}/voluntarios") }}"><img class="octicon" src="{{ URL::asset('icons/person.svg') }}"></button>
							@endif
						@endif
